  ...
  return (
    <div className="max-w-3xl mx-auto space-y-6">
      <Card className="p-6">
        ...
        {!user ? (
          <p className="mt-2 text-sm text-ink-600">Please log in to manage your settings.</p>
        ) : (
          <div className="mt-4 space-y-4 text-sm">
            ...
            <div className="text-ink-500">Auth Token</div>
            <div className="font-mono break-all bg-gray-50 border rounded p-2 text-xs">{token || 'No token'}</div>
            <p className="text-ink-600">More settings can be added here (password change, notifications preferences, privacy controls).</p>
          </div>
        )}
      </Card>
    </div>
